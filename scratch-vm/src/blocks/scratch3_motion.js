...
class Scratch3MotionBlocks {
    ...
    getMonitored () {
        return {
            ...
            motion_zposition: {
                isSpriteSpecific: true,
                getId: targetId => `${targetId}_zposition`
            },
            motion_direction: {
                isSpriteSpecific: true,
    ...
    goToXY (args, util) {
        ...
        const z = Cast.toNumber(typeof args.Z === 'undefined' ? util.target.z : args.Z);
    ...
    getTargetXY (targetName, util) {
        let targetX = 0;
        let targetY = 0;
        let targetZ = util.target.z || 0;
        if (targetName === '_mouse_') {
            targetX = util.ioQuery('mouse', 'getScratchX');
            targetY = util.ioQuery('mouse', 'getScratchY');
            targetZ = util.target.z || 0;
    ...
    goTo (args, util) {
        ...
        if (targetXY) {
            if (typeof util.target.setXYZ === 'function') {
                util.target.setXYZ(targetXY[0], targetXY[1], targetXY[2]);
            } else {
    ...
    glide (args, util) {
        if (util.stackFrame.timer) {
            ...
            if (timeElapsed < util.stackFrame.duration * 1000) {
                ...
                const dz = frac * (util.stackFrame.endZ - util.stackFrame.startZ);
                if (typeof util.target.setXYZ === 'function') {
                    util.target.setXYZ(
                        util.stackFrame.startX + dx,
                        util.stackFrame.startY + dy,
                        util.stackFrame.startZ + dz
                    );
                } else {
                    util.target.setXY(
                        util.stackFrame.startX + dx,
            ...
            util.stackFrame.startZ = util.target.z || 0;
            util.stackFrame.endX = Cast.toNumber(args.X);
            util.stackFrame.endY = Cast.toNumber(args.Y);
            util.stackFrame.endZ = Cast.toNumber(typeof args.Z === 'undefined' ? util.target.z || 0 : args.Z);
    ...
    glideTo (args, util) {
        const targetXY = this.getTargetXY(args.TO, util);
        if (targetXY) {
            this.glide({SECS: args.SECS, X: targetXY[0], Y: targetXY[1], Z: targetXY[2]}, util);
        }
    }
    ...
    changeZ (args, util) {
        ...
        if (typeof util.target.setZ === 'function') {
            util.target.setZ(util.target.z + dz);
        } else {
    ...
    getZ (args, util) {
        return this.limitPrecision(util.target.z || 0);
    }
