...
class RenderedTarget extends Target {
    ...
    constructor (sprite, runtime) {
        ...
        /**
         * Scratch3D Z coordinate. Positive values move actors deeper into the 3D scene.
         * @type {number}
         */
        this.z = 0;

        /**
         * Scratch3D model asset attached to this target.
         * @type {?string}
         */
     ...
     * The 2D renderer still receives X/Y; Z is stored for Scratch3D renderers.
     * @param {!number} x New X coordinate, in Scratch coordinates.
     * @param {!number} y New Y coordinate, in Scratch coordinates.
     * @param {!number} z New Z coordinate, in Scratch3D coordinates.
     * @param {?boolean} force Force setting X/Y/Z, in case of dragging
     */
    setXYZ (x, y, z, force) {
        if (this.isStage) return;
        if (this.dragging && !force) return;
    ...
    /**
     * Set only the Z coordinate.
     * @param {!number} z New Z coordinate, in Scratch3D coordinates.
     * @param {?boolean} force Force setting Z, in case of dragging
     */
    setZ (z, force) {
        if (this.isStage) return;
        if (this.dragging && !force) return;
    ...
    /**
     * Attach a Scratch3D model to this target.
     * @param {{id: string, name: string, dataUri: string}} model Model metadata.
     */
    setModel3D (model) {
        if (this.isStage) return;
    ...
    postSpriteInfo (data) {
        ...
        if (Object.prototype.hasOwnProperty.call(data, 'z')) {
            this.setZ(data.z, force);
        }
